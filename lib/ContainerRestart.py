...
class ContainerRestart:
    def __init__(self,listContainers,userOptions=None):
        ...
        self.ContainerRestart = self.ContainerRestart()

    def ContainerRestart(self):
        if self.userOptions != None and self.listContainers:
            ...
            for index, container in enumerate(self.listContainers):
                containerRestartCommand = Container.UserContainerOptionCommand(self.command,self.userOptions)
                ...
                if not User().newUser(container,self.json_file_path):
                    if Container.ContainerId(container,self.json_file_path):
                        ...
                        containerRestartCommand.append(id)
                        result = subprocess.run(containerRestartCommand, capture_output=True, text=True)
