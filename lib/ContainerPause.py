...
class ContainerPause:
    def __init__(self,listContainers,userOptions=None):
        ...
        self.command = ["docker", "container","pause"]
        self.ContainerPause = self.ContainerPause()
    ...
    def ContainerPause(self):
        if self.userOptions != None and self.listContainers:
            ...
            for index, container in enumerate(self.listContainers):
                containerPauseCommand = Container.UserContainerOptionCommand(self.command,self.userOptions)
                ...
                if not User().newUser(container,self.json_file_path):
                    if Container.ContainerId(container,self.json_file_path):
                        ...
                        containerPauseCommand.append(id)
                        result = subprocess.run(containerPauseCommand, capture_output=True, text=True)
