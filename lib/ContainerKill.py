...
class ContainerKill:
    def __init__(self,listContainers,userOptions=None):
        ...
        self.command = ["docker", "container","kill"]
        self.ContainerKill = self.ContainerKill()

    def ContainerKill(self):
        if self.userOptions != None and self.listContainers:
            ...
            for index, container in enumerate(self.listContainers):
                containerKillCommand = Container.UserContainerOptionCommand(self.command,self.userOptions)
                ...
                if not User().newUser(container,self.json_file_path):
                    if Container.ContainerId(container,self.json_file_path):
                        ...
                        containerKillCommand.append(id)
                        result = subprocess.run(containerKillCommand, capture_output=True, text=True)
